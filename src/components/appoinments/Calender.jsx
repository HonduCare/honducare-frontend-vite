...
const Calender = () => {
  ...
  const obtenerCitasDelDia = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_REACT_APP_API_URL}/obtener/citas`);
    const citas = response.data.map((cita) => ({
        ...
        extendedProps: {
          ...
          id: cita.id_cita, // ID de la cita para editar
                  ...
                  <div className="card-body">
                    <div id="calendar">
                      <FullCalendar
                        ...
                        eventClick={handleEventClick} // Manejar clic en una cita
                      />
                    </div>
                  </div>
                </div>
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              ...
              <div className="modal-body">
                <p><strong>Paciente:</strong> {selectedEvent.paciente.nombre_completo}</p>
                <p><strong>Teléfono:</strong> {selectedEvent.paciente.telefono}</p>
                <p><strong>Numero de identificación:</strong> {selectedEvent.paciente.numero_identidad}</p>
                <p><strong>Motivo:</strong> {selectedEvent.motivo}</p>
              </div>
              <div className="modal-footer">
                <Link
                  to={`/EditCita/${selectedEvent.id}`} // Ruta para editar la cita

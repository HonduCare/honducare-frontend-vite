  ...
  const obtenerCargos = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_REACT_APP_API_URL}/obtener/cargos`
      );
    ...
    try {
      if (mode === "crear") {
        const response = await axios.post(
          `${import.meta.env.VITE_REACT_APP_API_URL}/crear/cargo`,
          data
        );
        ...
        const response = await axios.put(
          `${import.meta.env.VITE_REACT_APP_API_URL}/actualizar/cargo/${
            cargo.id_cargo
          }`,
          data
        );
  ...
  const eliminarCargo = async (id) => {
    try {
      const response = await axios.delete(
        `${import.meta.env.VITE_REACT_APP_API_URL}/eliminar/cargo/${id}`
      );
      if (response.status === 200) {
            ...
            <div className="dropdown-menu dropdown-menu-start">
              ...
              {hasUpdatePermission && (
                <button
                  ...
                  onClick={() => handleEditCargo(record)}
                >
              ...
              {hasDeletePermission && (
                <button
                  className="dropdown-item"
                  onClick={() => eliminarCargo(record.id_cargo)}
                >

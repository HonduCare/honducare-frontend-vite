...
const ListaSexos = () => {
  ...
  const obtenerSexos = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_REACT_APP_API_URL}/obtener/sexo/sexos`);
  ...
  const onSubmit = async (data) => {
    try {
      if (mode === 'crear') {
          const response = await axios.post(`${import.meta.env.VITE_REACT_APP_API_URL}/crear/sexo`, data);
          ...
          const response = await axios.put(`${import.meta.env.VITE_REACT_APP_API_URL}/actualizar/sexo/${sexo.id_sexo}`, data);
          if (response.status === 200) {
            Swal.fire({
  ...
  const eliminarSexo = async (id) => {
    try {
      const response = await axios.delete(`${import.meta.env.VITE_REACT_APP_API_URL}/eliminar/sexo/${id}`);
      if (response.status === 200) {
        Swal.fire({

...
const ListaDocumento = () => {
  ...
  const obtenerTipoD = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_REACT_APP_API_URL}/obtener/tipoDocumento/tiposDocumentos`);
  ...
  const onSubmit = async (data) => {
    try {
      if (mode === "crear") {
        const response = await axios.post(`${import.meta.env.VITE_REACT_APP_API_URL}/crear/tipoDocumento`, data);
        ...
        const response = await axios.put(`${import.meta.env.VITE_REACT_APP_API_URL}/actualizar/tipoDocumento/${documento.id_documento}`, data);
        if (response.status === 200) {
          Swal.fire({
            icon: 'success',
  ...
  const eliminarTipoD = async (id) => {
    try {
      const response = await axios.delete(`${import.meta.env.VITE_REACT_APP_API_URL}/eliminar/tipoDocumento/${id}`);
      if (response.status === 200) {
        Swal.fire({
        ...
        <div className="text-start">
          <div className="dropdown dropdown-action">
            ...
            <div className="dropdown-menu dropdown-menu-start">
              <button className="dropdown-item" onClick={() => handleEditTipoD(record)}>
                <i className="far fa-edit me-2" /> Editar
              </button>
              <button className="dropdown-item" onClick={() => eliminarTipoD(record.id_documento)}>
  ...
  return (
    <>
      ...
      <Modal
        title={mode === "editar" ? "Editar Tipo de Documento" : "Crear Tipo de Documento"}

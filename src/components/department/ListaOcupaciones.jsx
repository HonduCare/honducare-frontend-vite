...
const ListaOcupaciones = () => {
  ...
  const obtenerOcupaciones = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_REACT_APP_API_URL}/obtener/ocupaciones/ocupacion`);
  ...
  const onSubmit = async (data) => {
    try {
      if (mode === 'crear') {
        const response = await axios.post(`${import.meta.env.VITE_REACT_APP_API_URL}/crear/ocupaciones/`, data);
        ...
        const response = await axios.put(`${import.meta.env.VITE_REACT_APP_API_URL}/actualizar/ocupaciones/${ocupacion.id_ocupacion}`, data);
        if (response.status === 200) {
          Swal.fire({
  ...
  const eliminarOcupacion = async (id) => {
    try {
      const response = await axios.delete(`${import.meta.env.VITE_REACT_APP_API_URL}/eliminar/ocupaciones/${id}`);
      if (response.status === 200) {
        Swal.fire({
      ...
      render: (text, record) => (
        <div className="text-start">
          <div className="dropdown dropdown-action" >
            <Link
              to="#"
              className="action-icon dropdown-toggle" data-bs-toggle="dropdown"
              aria-expanded="false">
            ...
            <div className="dropdown-menu dropdown-menu-start">
              <button
                ...
                onClick={() => handleEditOcupacion(record)} style={{ padding: '0.25rem 1rem' }}>
                <i className="far fa-edit me-2" />
                Editar
              </button>
              <button
                className="dropdown-item"
                onClick={() => eliminarOcupacion(record.id_ocupacion)} style={{ padding: '0.25rem 1rem' }}>
  ...
  return (
    <>
      <Header />
      <Sidebar id='menu-item6' id1='menu-items6' activeClassName='lista-ocupaciones' />
            ...
            <div className="row">
              <div className="col-sm-12">
                <ul className="breadcrumb">
                  ...
                  <li className="breadcrumb-item">
                    <i className="feather-chevron-right"><FeatherIcon icon="chevron-right" /></i>
                  </li>
        ...
        <Form layout="vertical" onFinish={handleSubmit(onSubmit)}>
          {mode === 'editar' && (
            <Form.Item label="ID Ocupación">
              <Controller name="id_ocupacion" control={control} render={({ field }) => <Input {...field} disabled />} />
            </Form.Item>
          )}
          <Form.Item label="Nombre de la Ocupación" rules={[{ required: true }]}>
